...
public class SettingsModel implements TreeModel, ModifyAndSaveObject {
    ...
    public void setRootSettings(Settings newRootModel) {
        ...
        logger.fine("This rootSettings contains " +
            ...
            "User info: [ " + rootSettings.getUserInfo() + " ]"// Some old settings files have no parameters and cause a null exception on this line
    ...
    private void addFileListModelForOiTarget(Oitarget target, File file) {
        ...
        if (!lm.contains(file)) {
            logger.fine("adding file " + file.getName() + " to listmodel for " +
                key);
            lm.addElement(file);
        } else {
    ...
    public void saveSettingsFile(java.io.File fileToSave,
        ...
        URL mappingURL = this.getClass().getClassLoader()
                             .getResource("jmmc/mf/gui/mapping.xml");
    ...
    public int getChildCount(Object parent) {
        ...
        if (parent instanceof Settings) {
            ...
            if (s.getFiles().getFileCount() >= 1) {
                ...
                logger.fine("target count is " +
                    s.getTargets().getTargetCount());
                i++;
            }
